...
public class AfkTracker {
    ...
    private static final int absentTimeTrigger = RccEssentials.CONFIG.afk.afkTimeTrigger * 20; // seconds * 20 ticks
    ...
    public static void register() {
        ...
        Placeholders.register(new Identifier(RccEssentials.MOD_ID, "afk"), (context, argument) -> {
        ...
        PlayerActivityEvents.AFK.register((player, server) -> {
            RccEssentials.LOGGER.info("{} is AFK. Active time: {} seconds.", player.getGameProfile().getName(), getInstance().getActiveTime(player));

            var playerContext = PlaceholderContext.of(player);

            RccEssentials.getInstance().broadcast(Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.afk.goneAfk,
                    playerContext
            ));
        });

        PlayerActivityEvents.AFK_RETURN.register((player, server) -> {
            RccEssentials.LOGGER.info("{} is no longer AFK. Active time: {} seconds.", player.getGameProfile().getName(), getInstance().getActiveTime(player));

            var playerContext = PlaceholderContext.of(player);

            RccEssentials.getInstance().broadcast(Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.afk.returnAfk,
                    playerContext
            ));
        });
