...
public class TeleportAcceptCommand {
    ...
    private static void execute(CommandContext<ServerCommandSource> context, TeleportTracker.TeleportRequest request) {
        ...
        if (sourcePlayer == null || targetPlayer == null) {
            context.getSource().sendFeedback(() -> Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.teleportRequest.playerUnavailable,
                    playerContext
            ), false);
            return;
        }
        ...
        if (player.getUuid().equals(request.target)) {
            ...
            context.getSource().sendFeedback(() -> Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.teleportRequest.requestAcceptedResult,
                    playerContext
            ), false);
            sourcePlayer.sendMessage(Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.teleportRequest.teleporting,
                    sourceContext
            ), false);
        } else {
            ...
            targetPlayer.sendMessage(Placeholder.parse(
                    RccEssentials.CONFIG.textFormats.commands.teleportRequest.requestAccepted,
                    targetContext,
